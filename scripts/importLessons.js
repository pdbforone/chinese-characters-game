...
`;
  ...
  lessonNumbers.forEach((num) => {
    ...
    lesson.forEach((char, idx) => {
      if (!char.character) {
        console.error(`❌ Lesson ${num}, Index ${idx}: Missing character`);
        hasErrors = true;
      }
      if (!char.story || char.story.length < 10) {
        console.warn(`⚠️  Lesson ${num}, Character ${char.character}: Story too short or missing`);
        warningCount++;
      }
      if (char.tone < 1 || char.tone > 5) {
        console.error(`❌ Lesson ${num}, Character ${char.character}: Invalid tone ${char.tone}`);
        hasErrors = true;
      }
    });
  });
  ...
  spotCheckLessons.forEach((num) => {
    if (num === '...') {
      ...
      const count = lessonGroups[num].length;
      const chars = lessonGroups[num].map((c) => c.character).join('');
